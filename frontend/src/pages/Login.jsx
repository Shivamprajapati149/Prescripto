...
const Login = () => {
  ...
  const[State,setState] = useState('Sign Up') //initially state is sign up
  ...
  const onSubmitHandler = async (event) => {
    event.preventDefault()  //for not page reload
    // यहाँ आप login logic लिखेंगे

     try{
      if(State === 'Sign Up'){

        //register krege user ko
  ...
  return (
    <form  onSubmit={onSubmitHandler} className='min-h-[80vh] flex items-center'>
      <div className='flex flex-col gap-3 m-auto items-start p-8 min-w-[340px] sm:min-w-96 border rounded-xl text-zinc-600 text-sm shadow-lg  border-gray-200 '>
          ...
          <button type='submit' className='bg-[#5f6FFF] text-base py-2 rounded text-white font-semibold w-full cursor-pointer '>{State ==='Sign Up' ? " Create Account" : "Login"}</button>
          {
            State === 'Sign Up' ?
            <p className='text-sm text-zinc-500'>Already have an account? <span onClick={()=>setState('Login')} className='text-[#5f6FFF] underline cursor-pointer'>Login here</span>  </p>
